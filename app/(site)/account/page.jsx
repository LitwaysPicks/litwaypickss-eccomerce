...
function AccountContent() {
  ...
  // userInfo state is populated from `user` when the user opens the edit form.
  // No effect needed — initialization happens in the Edit button's onClick handler.
  ...
  const handleRetryOrder = async (order) => {
    ...
    try {
      ...
      if (unavailable.length > 0) {
        toast.warning(
          unavailable.length === order.items?.length
      ...
      available.forEach((product) => addToCart(product, product.quantity));
      toast.success("Items added to cart — redirecting to checkout…");
      setTimeout(() => router.push("/checkout"), 800);
    } catch {
      toast.error("Could not check item availability. Please try again.");
    } finally {
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    ...
                    <input
                      type="text"
                      name="firstName"
                      value={isEditing ? userInfo.firstName : (user?.first_name ?? "")}
                  ...
                  <div>
                    ...
                    <input
                      type="text"
                      name="lastName"
                      value={isEditing ? userInfo.lastName : (user?.last_name ?? "")}
                      onChange={handleInputChange}
                      disabled={!isEditing}
                      className="input disabled:bg-gray-50"
                    />
                  ...
                  <div>
                    ...
                    <input
                      type="tel"
                      name="phone"
                      value={isEditing ? userInfo.phone : (user?.phone ?? "")}
                      onChange={handleInputChange}
                      disabled={!isEditing}
                      className="input disabled:bg-gray-50"
                    />
                  ...
                  <div className="md:col-span-2">
                    ...
                    <input
                      type="text"
                      name="address"
                      value={isEditing ? userInfo.address : (user?.address ?? "")}
                      onChange={handleInputChange}
                      disabled={!isEditing}
                      className="input disabled:bg-gray-50"
                    />
                  ...
                  <div>
                    ...
                    <input
                      type="text"
                      name="city"
                      value={isEditing ? userInfo.city : (user?.city ?? "")}
                      onChange={handleInputChange}
                      disabled={!isEditing}
                      className="input disabled:bg-gray-50"
                    />
                  ...
                  <div>
                    ...
                    <select
                      name="county"
                      value={isEditing ? userInfo.county : (user?.country ?? "")}
                      onChange={handleInputChange}
                      disabled={!isEditing}
                      className="input disabled:bg-gray-50"
                    >
                        ...
                        >
                          <div className="flex items-center justify-between mb-3">
                            <div className="flex items-center space-x-4">
                              ...
                              <span
                                className={`px-2 py-1 rounded-full text-xs font-medium ${statusStyles[order.payment_status] ?? "text-gray-600 bg-gray-50"}`}
                              >
                                {order.payment_status.charAt(0) +
                            ...
                            <div className="flex items-center gap-3">
                              ...
                              {order.payment_status === "FAILED" && (
                                <button
                                  ...
                                  className="flex items-center gap-1.5 rounded-lg bg-primary-600 px-3 py-1.5 text-xs font-semibold text-white hover:bg-primary-700 disabled:opacity-60 disabled:cursor-not-allowed transition-colors"
                                >
                                  <RefreshCw className={`h-3 w-3 ${retryingOrderId === order.id ? "animate-spin" : ""}`} />
                                  {retryingOrderId === order.id ? "Checking…" : "Retry Order"}
                                </button>
                              )}
                            </div>
                          </div>
                    ...
                    {[
                      {
                        ...
                        desc: "Receive order updates and promotions",
                        defaultChecked: true,
                      },
                      {
                        label: "SMS Notifications",
                        desc: "Get delivery updates via SMS",
                        defaultChecked: true,
                      },
                      {
          ...
          <div className="bg-white rounded-xl p-6 w-full max-w-md shadow-xl">
            ...
            <div className="bg-red-50 border border-red-200 rounded-lg p-4 mb-4">
              ...
              <p className="text-sm text-red-600 mt-1">
                Your profile, order history, and all data will be deleted.
              </p>
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">
