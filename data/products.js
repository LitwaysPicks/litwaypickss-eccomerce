...
// Get search suggestions — returns { text, type }[] ranked by relevance
export function getSearchSuggestions(query, limit = 8) {
  if (!query || query.trim() === "") return [];

  const q = query.toLowerCase().trim();

  // Score a string: 30 = exact, 20 = starts-with, 10 = contains, 0 = no match
  const scoreStr = (str) => {
    const s = str.toLowerCase();
    if (s === q) return 30;
    if (s.startsWith(q)) return 20;
    if (s.includes(q)) return 10;
    return 0;
  };

  // Map of text -> { type, score } — keeps the highest score per text
  const map = new Map();

  const add = (text, type) => {
    const s = scoreStr(text);
    if (s === 0) return;
    const existing = map.get(text);
    if (!existing || existing.score < s) map.set(text, { type, score: s });
  };

  products.forEach((product) => {
    add(product.name, "product");
    add(product.brand, "brand");

    const cat = categories.find((c) => c.slug === product.category);
    if (cat) add(cat.name, "category");

    product.tags?.forEach((tag) => add(tag, "tag"));
  });

  return Array.from(map.entries())
    .map(([text, { type, score }]) => ({ text, type, score }))
